...
//				sendBizMsg2TransctionCompensate();
	...
	public void sendBizExceptionMsgDisruptorConcurrenly(BaseLppzBizException exception) throws InterruptedException {
		if (exception != null && exception.getRollbackMsgs() != null 
				&& StringUtils.isNotBlank(exception.getRollbackMsgs().getKey())) {
			...
			param.setKey(exception.getRollbackMsgs().getKey());
			param.setBody(exception);
			param.setTag(StringUtils.isBlank(exception.getRollbackMsgs().getModule())?"bizException":exception.getRollbackMsgs().getModule());
